        ...
        if post_param.get('action') == "submit":
            ...
            if act == 'edit':
                ...
                if isinstance(msg, int):
                    if data['default'] == 1:
                        self.db.edit('address', {'default': 0}, 'uid=%s && id != %s' % (self.kg['user']['id'], address_id))
                    return alert(act='/user/my_address')
                else: return alert(msg='修改失败:%s' % msg, act=3)
            else:
                ...
                if msg:
                    ...
                    return alert(act='/user/my_address')
                else: return alert(msg='添加失败:%s' % msg, act=3)
