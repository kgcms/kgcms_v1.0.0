    ...
    def __call__(self):
        ...
        for order in order_list:
            if order['oid'].startswith('ES'):
                ...
                # 事务
                self.db.run_sql('UPDATE `%sorder` SET status=-4 WHERE id=%s' % (self.db.cfg["prefix"], order['id']), log=False)
                # 退还库存
        ...
        if action in ('del', 'pay', 'cancel'):
            ...
            if action == 'del':
                ...
                url = '/api/unlimited_process?action=pay&code=%s' % cipher(json2dict({'order_number': order_data['oid']}, trans=False), 1)
                    ...
                    if order_data['oid'].startswith('ES'):
                        ...
                        for i in range(len(order_data['attribute'])):
                            if order_data['pid'][i] not in skus:
                                skus[order_data['pid'][i]] = [{"speci": order_data['attribute'][i], "number": order_data['number'][i]}]
                            else:
                                skus[order_data['pid'][i]].append({"speci": order_data['attribute'][i], "number": order_data['number'][i]})
                        products = self.db.list(
                            table='product',
                        ...
                        for product in products:
                            ...
                            for row in edit_datas:
                                ...
                                product['inventory'][index] += row['number']
                        # 事务
                        self.db.run_sql('UPDATE `%sorder` SET status=-4 WHERE id=%s' % (self.db.cfg["prefix"], id), log=False)
                        # 退还库存
                        sql = 'UPDATE %sproduct SET inventory = CASE id ' % self.db.cfg["prefix"]
                        for product in products:
                            sql += 'WHEN %s THEN "%s" ' % (product['id'], json2dict(product['inventory']))
